...
class Index
{
    // 为什么区分度不高的字段不需要加索引？(唱吧) // todo 当前认识，后续有更深入的认识再更新
    # 区分度不高的字段如状态，性别,加索引的话先查索引然后回表查询，增加了额外的io消耗，不如直接查表来的好
    # 增加索引的维护，占用额外的空间
...
*/
         /** Hash索引和B+树所有有什么区别或者说优劣呢? */
          ...
          # hash索引不支持使用索引进行排序，hash索引不支持模糊查询以及多列索引的最左前缀匹配.原理也是因为hash函数的不可预测.
          # AAAA和AAAAB的索引没有相关性.Hash索引任何时候都避免不了回表查询数据
        ...
        /** 为什么列一般设置为not null？ */
        # 空值存储是不占用空间的，null是占用空间，浪费空间
        # 任何数值和null进行运算都是null，判断是否为null 要使用is null
        # null值使查询的优化更难，索引的运算更加复杂
        ...
        # 其中包含了许多信息. 可以通过其中和索引有关的信息来分析是否命中了索引,例如possilbe_key,key,key_len等字段,
        ...
        # 排他锁又称为写锁，简称X锁。顾名思义，排他锁就是不能与其他锁并存，如果一个事务获取了一个数据行的排他锁，其他事务就不能再获取该行的其他锁，
        # 包括共享锁和排他锁，但是获取排他锁的事务是可以对数据行读取和修改。
        ...
        # innodb 支持表锁、行锁， mysim只支持表锁
        # innodb 支持MVCC， mysim不支持
        # innodb 支持外键
        # myisam 支持全文索引，innodb不支持
        # myisam 会保存整表的记录行数， innodb不保存
        # myisam 是三个表文件，frm(表结构) myd(数据文件) myi(索引文件), innodb 是两个, frm(表结构) ibd(数据、索引文件)
        ...
        # 存储的是系统版本号，不是真实的时间。每开始一个新的事务，系统版本号都会自动递增。
        ...
        # 一张表，里面有ID自增主键，当insert了17条记录之后，删除了第15,16,17条记录再把Mysql重启，再insert一条记录,这条记录的ID是18还是15?
        ...
        # 第二范式(2NF)是在第一范式(1NF)的基础上建立起来的，即满足 第二范式(2NF)必须先满足第一范式(1NF)。
        # 要求数据库表中的每 个实例或行必须可以被惟一地区分。通常需要为表加上一个列，以存储 各个实例的惟一标识。这个惟一属性列被称为主关键字或主键。
        # 满足第三范式(3NF)必须先满足第二范式(2NF)。简而言之，第三 范式(3NF)要求一个数据库表中不包含已在其它表中已包含的非主关键字信息。
        # 所以第三范式具有如下特征: >>1. 每一列只有一个 值 >>2. 每一行都能区分。>>3. 每一个表都不包含其他表已经包含 的非主关键字信息。

        /** mysql 的复制原理以及流程？ */
        # 复制过程中一 个服务器充当主服务器，而一个或多个其它服务器充当从服务器。主服务器将 更新写入二进制日志文件，并维护文件的一个索引以跟踪日志循环。
        # 这些日志 可以记录发送到从服务器的更新。
        # 当一个从服务器连接主服务器时，它通知主 服务器在日志中读取的最后一次成功更新的位置。从服务器接收从那时起发生 的任何更新，然后封锁并等待主服务器通知新的更新。

        # 过程如下 ：
        # 主服务器 把更新记录到二进制日志文件中(bin log)。
        # 从服务器把主服务器的二进制日志拷贝 到自己的中继日志(replay log)中。
        # 从服务器重做中继日志中的时间， 把更新应用到自己的数据库上。

        # 每当有从库链接到主库时，主库创建一个线程像从库发送bin-log
        # 从库创建一个i/o线程读取主库的bin-log数据更新到本地文件, repay log
        # 从库创建的另外一个线程读取从库写的 repay log的更新事件并执行

        /** Mysql主从同步的步骤 */
        # 步骤
        # 主库更新事件，写入bin log
        # 从库发起连接到主库
        # 主库创建一个线程，发送bin log到从库
        # 从库启动后创建一个线程读取主库传来的bin log数据写入本地的repay log
        # 从库还会创建另外一个线程，从repay log里面读取内容，将更新内容写入从库 slave db

        /** 出具库主从同步的好处 */
        # 数据的备份，主数据库故障之后可以使用从数据库继续工作
        # 业务量大，单机无法支持的时候可以通过多个库分担压力
        # 读写分离，支持更大的并发，master负责写入， slave负责读取
        ...
        # 进行 count ()统计某列的记录数的时候，如果采用的 NULL 值，会被系统自动忽略掉，但是空值是统计到其中。
