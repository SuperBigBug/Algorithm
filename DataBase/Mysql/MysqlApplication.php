...
class MysqlApplication
{
    ...
    /** 跨系统数据实时同步如何实现? 分布式事务? */
    // 上面提到的使用Canal将服务伪装成Mysql的从节点接受binLog来写入redis, 但是从库众多的情况下存在第一写不过来
    // 第二、下游数据库也需要对数据进行过滤处理等操作

    // 在上下游之间增加Mq来解耦
    // Canal从Mysql接受binLog数据，解析成结构化的数据写入Mq, 由需要的业务方来订阅消费
    // 不同的业务订阅消费队列中的数据, 拿到数据后各自进行过滤、转换等操作并保存

    /** 如何不停机的更换数据库? */
    // 建好新库, 变更程序, 双写! 程序上线后的数据新旧库同时写入 .同步旧库的数据到新库
    // 双写程序做好开关: 只写新库、只写旧库、新旧双写
}
